...
# All rights reserved.
# ==============================================================================
'''This code represents decission architecture. The left and right stimulus envelopes were mixed in a single channel that was appended to the lagged EEG data matrix'''
...
def createModel():
    ...
    model.add(Dense(n_hidden**(5), input_shape=(Window10s, (numChans)*2), activation='relu', use_bias=True))   
...
for Subject in range(1,2):
    workingDir='' #Provide your working path here
    ...
    '''Training Dataset'''
    trainingDataEEG=np.zeros(((numBlocks-2),Window10s,numChans)) # 80% of data for training
...
io.savemat(workingDir+'/Results/RevMulti_'+str(Subject)+'.mat',{'predicted'+str(Subject):predicted, 'Growntrouth'+str(Subject):tested})    #'corrA':np.squeeze(corrA[str(aWindow)]),'corrU':np.squeeze(corrU[str(aWindow)])})
