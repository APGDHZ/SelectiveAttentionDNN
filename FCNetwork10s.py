...
# All rights reserved.
# ==============================================================================
'''This code represents fully connected neural network. EEG Data divided in 288 blocks, 10s each. 286 blocks are taken for training, one for validation and another one for testing.
...
def createModel():
    ...
    model.add(Dropout(dropout))
    model.add(Dense(1, activation='tanh'))  #Ouput layer with 1 neuron
...
n_hidden=2 
dropout=0.25
numBlocks=288          #EEG Signal divided in 288 blocks, 10s each
Window10s=640          #10s of EEGSignal
...
for Subject in range(1,2):
    ...
    for block in range(numBlocks):
        eegData[block,:,:]=transfer1['eeg'][block*Window10s:(block+1)*Window10s,:]
